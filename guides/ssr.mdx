---
title: "Server-Side Rendering with svelte5-router"
sidebarTitle: "SSR"
...
The recommended approach is to declare `url` as reactive state in your top-level component and only assign it a real value when running on the server:
...
<script>
  ...
  // In the browser this stays "", which is falsy and therefore ignored.
...
<script>
  ...
  // Receives the server request URL; falsy in the browser
  export let url = $state("");
</script>
